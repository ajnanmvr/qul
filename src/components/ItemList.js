...
export default function ItemList({ items, searchQuery, maxSuggestions }) {
  ...
  const handleDownloadPDF = (item) => {
    ...
    const backgroundImageUrl = "/certificate.jpg";
    ...
    fetch(backgroundImageUrl)
      ...
      .then((blob) => {
        ...
        const capitalizedPosition = item.position
          ? item.position.toUpperCase()
          : "";
        const capitalizedProgramme = item.programme.toUpperCase();

        doc.text(
          `This is to certify that Mr. ${capitalizedName} has been awarded`,
          pdfWidth / 2,
          135,
          { align: "center" }
        );
        if (item.position && item.grade) {
          doc.text(
            `the ${capitalizedPosition} position with ${capitalizedGrade} grade in ${capitalizedProgramme} in KAFAN'24`,
            pdfWidth / 2,
            144,
            { align: "center" }
          );
          ...
          doc.text(
            `the ${capitalizedPosition} position without any grade in ${capitalizedProgramme} in KAFAN'24`,
            pdfWidth / 2,
            144,
            { align: "center" }
          );
          ...
          doc.text(
            `${capitalizedGrade} grade without any position in ${capitalizedProgramme} in KAFAN'24`,
            pdfWidth / 2,
            144,
            { align: "center" }
          );
        ...
        doc.text(
          `DHIU PG Arts Fest organized by Darul Huda Students' Union (DSU)`,
          pdfWidth / 2,
          153,
          { align: "center" }
        );
        doc.text(
          `on October 01 to 06, 2024 at Darul Huda Islamic University`,
          pdfWidth / 2,
          162,
          { align: "center" }
        );
                    ...
                    <div>
                      ...
                      {item.position && item.grade ? (
                        <p className="text-gray-800">
                          <span className="font-semibold  p-0.5 pl-2 pr-2 bg-slate-200 rounded-md">
                            {item.position}
                          </span>{" "}
                          with{" "}
                          <span className="font-semibold  p-0.5 pl-2 pr-2 bg-slate-200 rounded-md">
